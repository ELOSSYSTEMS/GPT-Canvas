...
{% capture email_body %}
  לא קיבלת את ההזמנה שלך? <a href="mailto:{{ shop.email }}">עדכן אותנו</a>.
{% endcapture %}
    ...
    <table class="row actions">
      <tr>
        <td class="actions__cell">
          <table class="button main-action-cell">
            <tr>
              <td class="button__cell"><a href="{{ shop.url }}" class="button__text">בקרו בחנות שלנו</a></td>
            </tr>
          </table>
        </td>
      </tr>
...
{% endif %}
  ...
  <p class="disclaimer__subtext">
    ...
    {% if fulfillment.tracking_numbers.size == 1 and fulfillment.tracking_company and fulfillment.tracking_url %}
      מספר מעקב של {{ fulfillment.tracking_company }}: <a href="{{ fulfillment.tracking_url }}">{{ fulfillment.tracking_numbers.first }}</a>
    {% elsif fulfillment.tracking_numbers.size == 1 %}
      מספר מעקב: {{ fulfillment.tracking_numbers.first }}
    {% else %}
      מספרי מעקב של {{ fulfillment.tracking_company }}:<br />
