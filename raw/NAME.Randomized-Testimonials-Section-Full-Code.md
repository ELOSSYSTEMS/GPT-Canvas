...
<style>
  #shopify-section-{{ section.id }} {
    --testimonials-font-size: calc(
      clamp(1.125rem, 1.064516129032258rem + 0.25806451612903225vw, 1.25rem)
      * {{ section.settings.font_size_factor }}
    );
  }
  ...
  /* Each dot */
  #shopify-section-{{ section.id }} .page-dots .tap-area {
    width: 8px;
    height: 8px;
    padding: 0;
    border-radius: 999px;
  ...
  /* Active dot */
  #shopify-section-{{ section.id }} .page-dots .tap-area[aria-current="true"] {
    background-color: rgba(0, 0, 0, 0.8);
    border-color: rgba(0, 0, 0, 0.8);
  }

  /* OPTIONAL: hide dots on small screens if 28 is too many */
    ...
    <div class="container">
      <div class="section-stack">
        ...
        <div class="testimonial-list">
          ...
          {%- comment -%}
            Randomize testimonial order (pseudo-shuffle) so they don't render in the block order.
            NOTE: Liquid doesn't support true random; this is the best theme-safe approach.
          {%- endcomment -%}
          {%- assign randomized_blocks = section.blocks | sort: 'id' | sort: 'id' -%}
          {%- assign testimonials_with_image = randomized_blocks | map: 'settings' | where: 'image' -%}
          ...
          <testimonial-carousel allow-swipe {% if section.settings.auto_rotate_between_testimonials %}autoplay="{{ section.settings.cycle_speed }}"{% endif %} class="testimonial-carousel" id="{{ carousel_id }}">
            ...
            {%- for block in randomized_blocks -%}
              <div class="testimonial-item text-center {% if forloop.first %}is-selected{% endif %}" {{ block.shopify_attributes }}>
                ...
                {%- if block.settings.image != blank or block.settings.avatar_url != blank or block.settings.author != blank -%}
                  <div class="v-stack justify-items-center gap-4 {% if testimonials_with_image.size > 0 %}md:hidden{% endif %}">
                    ...
                    {%- elsif block.settings.avatar_url != blank -%}
                      <img
                        src="{{ block.settings.avatar_url | escape }}"
                        alt="{{ block.settings.author | escape }}"
                        class="constrained-image md:hidden"
  ...
  "blocks": [
    {
      ...
      "settings": [
        ...
        },
        {
          ...
          "info": "Used when no image is selected. Paste a full https URL here."
        },
        {
          "type": "range",
          "id": "logo_max_width",
