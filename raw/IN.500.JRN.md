...
To reduce overwhelm, **questions are asked one by one**, using a conversational or visual drip-style interface. The user sees the next prompt only after responding to the current one.
...
- **Drip UX:** Questions are presented **one by one** to guide user gently through flow
- **Tone:** Calm, supportive, neutral (matches active behavior mode)
- **Export Options:**
  - Free/Pro: Download as .txt
  - **Pro+ only:** Export to Google Docs, Sheets, or Calendar (depending on entry type)
- Subtle reminder in interface: “Want to send this to your Google Suite? Upgrade to Pro+”
