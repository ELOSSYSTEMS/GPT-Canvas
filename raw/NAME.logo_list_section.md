      ...
      <div class="v-stack gap-8 justify-center items-center">
        {%- capture section_content -%}
          {%- for block in section.blocks -%}
            <div class="logo-list__item" role="group" aria-label="{{ 'general.accessibility.item_nth_of_count' | t: index: forloop.index, count: section.blocks.size }}" {{ block.shopify_attributes }} style="display: flex; justify-content: center; align-items: center; margin-top: 0; flex: 1 1 auto; max-width: {{ block.settings.icon_width }}px;">
              {%- liquid
              ...
              {%- if block.settings.custom_icon != blank -%}
                ...
                {%- capture style -%}--image-mobile-max-width: {{ block.settings.mobile_icon_width }}px; --image-max-width: {{ block.settings.icon_width }}px; display: block; margin: auto; max-width: 100%;{%- endcapture -%}
        ...
        <div class="logo-list text-with-icons--stacked" role="region" style="display: flex; flex-wrap: {% if section.settings.stack_on_mobile %}wrap{% else %}nowrap{% endif %}; justify-content: center; align-items: center; gap: 20px; flex-wrap: wrap;">
          {{- section_content -}}
        </div>
      </div>
    </div>
