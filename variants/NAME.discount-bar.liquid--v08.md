      ...
      <div class="page-width text-center">
        <p class="prose heading announcement-bar__message">
          ...
          {% if section.settings.secondary_button_text and section.settings.secondary_button_link %}
            <a class="discount-bar__link-button" href="{{ section.settings.secondary_button_link }}" onclick="hideDiscountBar()">
...
function hideDiscountBar() {
  localStorage.setItem('ff_discount_bar_dismissed', 'true');
  const bar = document.querySelector('.discount-bar');
  if (bar) bar.style.display = 'none';
}

document.addEventListener('DOMContentLoaded', function () {
  if (localStorage.getItem('ff_discount_bar_dismissed') === 'true') {
    const bar = document.querySelector('.discount-bar');
    if (bar) bar.style.display = 'none';
  }
});
