...
function showComparison(prompt, gpt, dabroo) {
  const msg = document.createElement('div');
  msg.classList.add('message', 'system', 'visible');
  msg.innerHTML = `
    <div style="font-weight: bold; margin-bottom: 4px;">שאלה:</div>
    <div style="margin-bottom: 8px;">${prompt}</div>
    <div style="font-size: 0.85rem; opacity: 0.7;">🎛️ GPT רגיל:</div>
    <div style="background: #2e2e2e; padding: 12px; border-radius: 8px; margin-bottom: 12px;">${gpt}</div>
    <div style="font-size: 0.85rem; opacity: 0.7;">🌿 Dabroo:</div>
    <div style="background: #0a7aff33; padding: 12px; border-radius: 8px;">${dabroo}</div>
  `;
  chat.appendChild(msg);
  scrollToBottom(msg);
}
