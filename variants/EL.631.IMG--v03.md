...
const BrandPersona = {
  inject(prompt, brand = "") {
    ...
    if (knownBrands[brand]) {
      ...
      return `${prompt}\nStyle the image with visual elements aligned with the aesthetic identity of ${brand}. Search for '${brand} product photography' on Google Images or Bing. If available, analyze visual mood using Luma or similar model-based inference.`;
    }
  }
};