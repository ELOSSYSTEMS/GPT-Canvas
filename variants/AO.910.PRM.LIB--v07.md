...
class PersonalizedPromptAgent:
    def __init__(self, user_profile, prompt_library):
        self.user_profile = user_profile  # Dict: roles, goals, tone prefs, languages
        self.prompt_library = prompt_library  # Full AO.910.PRM.LIB reference
        self.trail_log = {}
    ...
    def filter_prompts(self):
        ...
        filtered = [p for p in self.prompt_library if domain in p["tags"] or "universal" in p["tags"]]
        return filtered
    ...
    def generate_bundle(self):
        ...
        bundle_id = f"user_bundle_{self.user_profile['user_id']}"
        return {"bundle_id": bundle_id, "prompts": adapted}
...
# Tutorial UI Bubble Logic (appears when new bundle is created)
def generate_tutorial_bubble():
    return {
        "title": "💡 Personalized Prompts Activated",
        "text": "Your library now evolves based on your profile. Rate prompts to optimize further.",
...
# Instantiate Primary Agent (to be activated via ELos bootloader)
prompt_agent_instance = PersonalizedPromptAgent(
    user_profile={"user_id": "alpha001", "roles": ["founder"], "tone": "sharp"},
    prompt_library=[]  # To be injected at runtime
)
